...
| # | Failure | Fix | Status |
|---|---------|-----|--------|
| 1 | The prompt named no song, artist, tempo, or language. The mood spread was four fixed strings shared by every artist in the catalogue. | Song-aware defaults + **editable prompts** | built |
...
```
brief  ──▶ plan_from_recipe ──▶ [ShotSpec]  ──▶ apply_overrides ──▶ [ShotSpec]
                                                                        │
                                                        _resolve() ─────┤
                                                                        │
...
Five ship: **backdrop loop** (silent, nobody in it), **direct address** (selfie, speaking,
fixed length, front reference), **performance** (cinematic, three-quarter reference),
**portrait still** (an image, cheap, reusable as a reference), and **voice-over** (one
spoken line as audio).

Five things the split settled that were wrong while it was code:
...
- **Whether the master goes under the clip is per format** (`score_with_master`, added
  2026-08-03). `services/scoring` maps the record over the provider's track exhaustively —
  `-map 0:v:0 -map 1:a:0`, so the model's audio is absent rather than muted. That is exactly
  right for a picture that sits under a record and exactly wrong for a direct-address clip,
  which is generated by paying a model to *speak a line*: scoring deleted the sentence and
  handed back a mute performance with the song over it. Checked on the delivery path too —
  `scored_now` exists for kits the worker did not score, which is the state such a clip is
  now deliberately in.
- **The shape of the frame is per format** (`aspect_ratio`, made real 2026-08-03). The field
  existed and reached the provider's params, while every prompt template still typed
  "vertical 9:16" into its prose and the identity-lock prelude still hardcoded vertical
  regardless of the clip it was the first frame of. A format re-shaped to 16:9 got the
  metadata it asked for wrapped around a vertical picture. The ratio now renders into the
  prompt as `{aspect}` from that same field, and the review grid letterboxes rather than
  centre-cropping to 9:16.
