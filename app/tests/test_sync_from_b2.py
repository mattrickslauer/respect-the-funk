...
def test_documents_only_by_default():
    ...
    copied, skipped, _ = sync(source, dest, PREFIX)
...
def test_media_is_opt_in():
    ...
    copied, _, total = sync(source, dest, PREFIX, media=True)
...
def test_a_dry_run_reads_nothing():
    ...
    copied, _, total = sync(source, dest, PREFIX, dry_run=True)
...
def test_one_unreadable_object_does_not_abort_the_rest():
    ...
    copied, _, _ = sync(source, dest, PREFIX)
...
def test_force_recopies_what_is_already_local():
    ...
    copied, skipped, _ = sync(source, dest, PREFIX, force=True)
