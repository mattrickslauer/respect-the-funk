...
`git show 14073d9:apps/spindle/web/spindle/demo.py`.
...
Secrets go in `infra/terraform/spindle/terraform.tfvars`, which is gitignored — not in `-var`
flags, because command-line arguments are visible in the process table to anything
running on the machine.

```hcl
# infra/terraform/spindle/terraform.tfvars
...
```bash
./infra/terraform/spindle/build.sh              # vendors arm64 wheels into infra/build
terraform -chdir=infra/terraform/spindle init
terraform -chdir=infra/terraform/spindle apply  # read the plan; it is the resource list
terraform -chdir=infra/terraform/spindle output sign_in_mail_configured   # false means nobody can sign in
```
...
deleted, only unshipped — `git show a6ba8bb:apps/spindle/schema/001_substrate.sql`, with its
deviations argued in `git show a6ba8bb:apps/spindle/README.md`. Reach for it when a piece
...
- **`ccloud` is installed, wired and has been run.** ~~Nobody has logged in yet.~~
  `apps/spindle/bin/ccloud-mcp-setup.sh` uses it to resolve the cluster ID by name and writes
