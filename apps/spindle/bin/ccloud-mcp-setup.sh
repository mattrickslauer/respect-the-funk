...
#
#     ./apps/spindle/bin/ccloud-mcp-setup.sh
#
...
#     need rather than a command run once to be able to say we ran it. `apps/spindle/README.md`
...
# `apps/spindle/bin` -> repository root is three up, not two. See dev.sh: the two-level
# form resolves to `apps/` and reads no `.env`, which surfaces as a cluster name that is
# silently the fallback rather than the one this deployment uses.
