"""Load documents into the spine, then let the fleet embed them.
    ...
    python -m spindle.ingest ../../docs/research/*.md
    python -m spindle.ingest --drain-only
    python -m spindle.ingest --search "what did we learn about creator rates"
...
def backfill_fcc_country(conn: psycopg.Connection, tenant_id: str) -> int:
    """Write `country_code = 'US'` for every FCC-licensed station that has none.

    ## Why this is a transcription and not a guess

    A party carrying an `fcc_facility_id` holds a Federal Communications Commission
    broadcast licence. The Commission licences US stations; `fcc.query` only ever asks it
    for US jurisdictions, and the FM Query row carries `US` in field 11 alongside the
    city and state that `index_stations` already stored as a `market` fact. So the
    country is not being inferred from the licence — it is a column of the same record
    every other fact on these rows was transcribed from, which is why this is written
    `measured` with `source = 'fcc_fm_query'` rather than `inferred`.

    Stating that plainly matters because `SCOPE-RESET §2a` forbids exactly the move this
    superficially resembles: deciding a fact from context and recording it as though it
    had been read. The distinction is that re-fetching the register would return the same
    string for every one of these rows, and the only thing skipping the fetch saves is
    8,184 HTTP requests for a value already determined by which endpoint answered them.

    ## Why only rows with no country at all

    `fact_one_live_per_dimension` is unique on `(tenant, party, dimension, provenance)`
    where `status = 'live'`, so a `measured` country would sit *alongside* the `asserted`
    one that `refresh_stream` writes for the ~384 stations Radio Browser also knows,
    rather than conflicting with it. Two live country facts on one party is not an error
    the database would catch and is an ambiguity every reader downstream would have to
    resolve, so the frontier below excludes any party that already has one — which is
    also why re-running this is free.
    """
    with conn.cursor() as cur:
        cur.execute(
            """INSERT INTO party_fact (tenant_id, party_id, dimension, value_text,
                                       provenance, status, source, written_by)
               SELECT i.tenant_id, i.party_id, 'country_code', 'US',
                      'measured', 'live', 'fcc_fm_query', 'backfill_fcc_country'
...
def refresh_streams(conn: psycopg.Connection, tenant_id: str) -> int:
    ...
    ## Why a finished bucket is re-opened rather than skipped

    `streams.REFRESH_CAP` bounds one lead to 200 stations, so a bucket holding three
    thousand of them needs fifteen passes. The original `ON CONFLICT … DO NOTHING` made
    the second pass impossible: the lead row from the first pass survives at
    `state = 'done'`, the conflict fires, nothing is queued, and the remaining stations
    are never looked at again. Measured on the live cluster 2026-08-15 — sixteen
    `refresh_stream` leads all `done`, and 33,051 counterparties still with no
    `country_code`, which is 91% of the Radio Browser index.

    The conflict now re-opens the row instead. That is safe precisely because the SELECT
    above is the frontier: a bucket only appears in `buckets` when it *still has stations
    with no country*, so re-opening a lead can never mean re-doing finished work. A
    bucket that genuinely is finished drops out of the SELECT and is never touched.

    `owner_agent`, `lease_expires_at` and `lease_token` are cleared as part of the same
    write. Leaving a stale lease on a row being handed back to the frontier would make it
    unclaimable until the lease lapsed, which would look exactly like the bug this fixes.
    `attempts` is deliberately *not* reset: it counts how often this bucket has failed,
    and a re-open is not evidence the failures did not happen.

    ## How many workers to run this with: two

    The buckets are independent and `fleet.claim` hands each to exactly one worker, so
    running several drainers is safe by construction. It is not free, though, and the
    limit is the cluster's rather than this code's. Measured on the production Basic
    cluster 2026-08-15: one lead is 200 sequential transactions and takes ~2.3 minutes,
    so throughput is round-trip-bound and extra workers do buy close to linear speedup —
    but **four workers exhausted the request-unit burst budget**, and while they ran,
    `/public/search` went from 1.6 seconds to over 200. The public map was unusable for
    as long as the backfill was fast.

    Two workers is the setting that finishes in reasonable time without throttling the
    surface the backfill exists to fill in. This is a property of a Basic cluster's RU
    budget, so re-measure rather than inherit the number if the cluster changes tier.
    """
    with conn.cursor() as cur:
        cur.execute(
        ...
        for bucket in buckets:
            cur.execute(
                """INSERT INTO lead (tenant_id, scope_kind, kind, mode, adapter,
                   ...
                   ON CONFLICT (tenant_id, target_hash) DO UPDATE
                      SET state = 'pending',
                          owner_agent = NULL,
...
def main() -> int:
    ...
    parser.add_argument("--backfill-fcc-country", action="store_true",
                        help="write country_code=US for FCC-licensed stations that have "
                             "none. No network call: the country is a column of the same "
                             "licence record every other fact on those rows came from.")
    ...
    with psycopg.connect(url, autocommit=True, row_factory=dict_row) as conn:
        ...
        if args.backfill_fcc_country:
            written = backfill_fcc_country(conn, tenant_id)
            print(f"wrote country_code for {written} FCC station(s)")
