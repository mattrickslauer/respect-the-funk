...
Jinja was the only reader. It stopped being correct when `spindle/api/` added a
second one.
...
from spindle.demo import Col, Field, Section, View, _bar
from spindle.domain import (
...
def _masters_sections(recording_id: str, assets: list[dict[str, Any]],
    ...
    """
    from spindle import assets as assets_mod
...
def imports(conn: psycopg.Connection, tenant_id: str) -> View:
    from spindle import statements
...
def implemented_agents() -> set[str]:
    ...
    """
    try:
        from spindle.agents import REGISTRY
        return set(REGISTRY)
...
def justification_sections(conn: psycopg.Connection, tenant_id: str,
    ...
    from spindle import agents, outreach
