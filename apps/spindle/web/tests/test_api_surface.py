...
class TheChannelSetMatchesTheSchema(unittest.TestCase):
    ...
    def _migration(self) -> str:
        # `apps/spindle/web/tests` -> `apps/spindle`. Anchored on the app root rather
        # than climbing to the repository root and back down through a named directory:
        # the old form counted three levels up and then re-entered `platform/`, so moving
        # the tree broke it twice over, and the failure was a missing file rather than a
        # wrong path — which reads as "the migration was deleted".
        path = Path(__file__).resolve().parents[2] / "schema" / "010_outreach.sql"
