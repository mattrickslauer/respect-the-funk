...
from rtf_platform import fleet, outreach
...
def core_statement() -> str:
    ...
    `platform/infra/main.tf` writes out SSE-S3: a default is a thing that can change, and
...
def fleet_worker(agent_name: str) -> Worker:
    ...
    """
    from rtf_platform import agents  # local: keeps the parsing half importable alone
...
def outbox_sender(worker_name: str) -> Sender:
    ...
    """
    from rtf_platform import sender as sender_mod
...
def lambda_handler(event: dict[str, Any], context: Any = None) -> dict[str, Any]:
    ...
    **Non-2xx is the retry signal and it is used on purpose.** CockroachDB re-delivers a
    ...
    """
    from rtf_platform import db, settings as settings_mod
...
def main(argv: Sequence[str] | None = None) -> int:
    """`python -m rtf_platform.changefeed [--verify|--dry-run|--follow]`.
