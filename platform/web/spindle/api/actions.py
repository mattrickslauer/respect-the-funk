...
real thing rather than trust a list.
  ...
  * **Every route is gated.** `deps.Writer`, which depends on `deps.require_signed_in`.
  ...
  * **Tenant scoping.** `deps.current_tenant` resolves it once per request and every
    ...
    this package along with the rest of `spindle/` and would fail on any statement
...
from spindle import agents, assets, outreach, repo, research
from spindle.api import errors
from spindle.api.deps import Conn, Tenant, Writer, not_allowed, not_found
